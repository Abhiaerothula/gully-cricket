...
export default function PlayersPage({css,isDark,teamsDB,setTeamsDB,premium,setPremium}){
  ...
  return(
    <div className="page-pad" style={{display:'flex',flexDirection:'column',gap:16}}>
        ...
        {teamPls.map((p,i)=>{const sr=p.balls>0?((p.runs/p.balls)*100).toFixed(0):0;const isSel=selectedPlayerId===p.id;return(
          <div key={p.id} onClick={()=>setSelectedPlayerId(p.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 10px',borderBottom:i<teamPls.length-1?`1px solid ${css.border}`:'none',borderRadius:10,cursor:'pointer',background:isSel?`${C.yellow}11`:'transparent',border:isSel?`1px solid ${C.yellow}44`:'1px solid transparent'}}>
            ...
            <div style={{display:'flex',gap:6}}>
              <button onClick={e=>{e.stopPropagation();openEdit(p)}} style={{background:`${C.yellow}22`,border:`1px solid ${C.yellow}33`,borderRadius:8,padding:8,cursor:'pointer',color:C.yellow,display:'flex',alignItems:'center'}}><Edit3 size={14}/></button>
              <button onClick={e=>{e.stopPropagation();delPlayer(p.id)}} style={{background:`${C.danger}22`,border:`1px solid ${C.danger}33`,borderRadius:8,padding:8,cursor:'pointer',color:C.danger,display:'flex',alignItems:'center'}}><Trash2 size={14}/></button>
            </div>
          </div>
        )})}
      </GSection>)}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:14}}>
      {selectedPlayer&&(<GSection title="🧾 PLAYER FULL STATS" css={css}>
